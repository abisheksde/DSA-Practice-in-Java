...
public class ObjectFinding {
    public static boolean isAnyObject(int[] from, int[] to) {
        ...
        // ***ERROR : Try to Find the Program Flow Accurately. Because, after 2nd
        // if-condition, it is returning the true, but not go out of the Method */
        // ***ERROR : Above result variable cannot be accessible within the 1st & 2nd
        // If-Condition*/
        ...
        if (fromI > toI && fromJ == toJ) {
            ...
            for (int i = fromI; i > toI; i--) {
                if (Board.board[i][toJ] != 0) {
                    objectFound = true;
                    break; // Object found
                }

            }
            return objectFound; // If Object Found --> true ///// If No object found -- false
        } else {
            return false;
        }
